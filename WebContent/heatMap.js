/**
 * 以特殊高亮的形式直观展示数据分布状况。
 * 注：chrome、safari、IE9及以上浏览器，核心的代码主要来自于第三方heatmap.js。@author juyang
 */
T.HeatmapOverlay = T.Overlay.extend({
    ...
    setDataSet: function (data) {
        ...
        while (dlen--) {
            var latlng = new T.LngLat(d[dlen][this.conf.lngField], d[dlen][this.conf.latField]);
            this.latlngs.push({
                latlng: latlng,
                c: d[dlen].count
            });
    ...
    /*==============================以上部分为专为天地图打造的覆盖物===================================================*/
    /*==============================以下部分为heatmap.js的核心代码,只负责热力图的展现====================================*/
